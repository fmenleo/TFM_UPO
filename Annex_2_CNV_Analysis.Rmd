---
title: 'ANNEX 2: CNV analysis via inferCNV (Burdziak et all. dataset)'
output:
  html_document:
    df_print: paged
---
...
First, I determine and confirm clusters of interest: acinar, ADM and PDAC cells.
...
tissue_cls_barplot
ggsave(plot = tissue_cls_barplot, filename = file.path(plot_dir, "barplot_cls_states.png"), height = 4, width = 6)
```
...
-   ADM cells from cls 7 can be considered as an early ADM,and come mainly from regenerative tissue (N2 conditions)

-   ADM cells from cls 3 can be considered as a later ADM state (CAE+KRAS), and come from pre-neoplasic and PanIN tissue

-   PDAC cells from cls 5 can be considered as an earlier PDAC, coming only from non-metastasic tumorigenic tissue
...
violin_plot
ggsave(plot = violin_plot,filename = file.path(plot_dir, "violinplot_cls.png"), height = 6, width = 6)
```
...
![Main heatmap output](images/infercnv.png)

![Heatmap with subclusters](images/infercnv_subclusters.png)
